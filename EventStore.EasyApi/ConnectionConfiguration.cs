...
namespace EventStore.EasyApi
{
    public class ConnectionConfiguration
    {
        ...
        {
            if (ConnectionString != null && !Hosts.Any(h => h.Host == ConnectionString.Host))
            {
                if (Port == DefaultPort && ConnectionString.Port > 0)
                    Port = (ushort)ConnectionString.Port;
                Hosts = Hosts.Concat(new[] { new HostConfiguration { Host = ConnectionString.Host } });
            }
            if (!Hosts.Any())
            {
                throw new ClientException("Invalid connection string. 'host' value must be supplied. e.g: \"host=myserver\"");
            }
